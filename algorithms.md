...
<img src="images/BookShelf.png" width="400">  <img src="images/Bookshelf Recreated.png" width="450">
...
The original implementation loaded textures into a fixed set of OpenGL texture slots. While functional, this approach introduced a limitation on the number of textures available in the scene at one time. Each texture required a dedicated slot, with only 16 being able to be bound at a time, which limited expansion and design and made future additions more difficult.
...
To address this issue, I removed the vector responsible for storing textures, and the affiliated methods and calls, and replaced it with an unordered map. At first, the plan was to bind textures during runtime, rather than all at once before rendering the scene, and simply replacing the active texture as each mesh was rendered. Unfortunately, the performance took a hit doing vector searches with every call to RenderScene, since searching by value with a vector has O(n) time complexity. This is why I chose an unordered map instead, which has O(1) time complexity on average, with O(n) as its worst-case. So, I set out refactoring all areas that called the vector, and began replacing them with calls to the unordered map instead.
...
Still, even though the vector searches were called with every rendering, and I improved that by calling the unordered map instead, I wanted to improve upon the fact that I was binding and unbinding textures repeatedly and redundantly during my RenderScene() call. My call to CreateBooks, for example, involved me binding and unbinding the “pages” texture multiple times, despite my knowing I would use it again in the next book.

<img src="images/CreateBooks.png" width="500">

<img src="images/CreateBookMethod.png" width="500">

So, I decided I could improve upon that by grouping the usages of each texture, and have them called together. This would mean all usages of the texture “pages” would be rendered at once, then that texture would be replaced with the next texture, and all of those usages would then be rendered, and so on. This removed the redundancy I recognized and was accomplished using another unordered map, but this one stored the instructions of each object to be rendered in a struct and assigned that struct to the texture it used as the key.
